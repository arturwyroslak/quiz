...
function VerifyAccountContent() {
  ...
  useEffect(() => {
    const verifyAccount = async () => {
      if (!token) {
        setStatus('error');
        setMessage('Brakujący token weryfikacyjny.');
        return;
      }
      ...
      try {
        ...
        if (response.ok) {
          setStatus('success');
          setMessage('Twoje konto zostało pomyślnie zweryfikowane. Możesz teraz się zalogować.');
          // Przekierowanie do strony logowania po 3 sekundach
          setTimeout(() => {
            router.push('/partner-program/login?verified=true');
          }, 3000);
        } else {
          const data = await response.json();
          setStatus('error');
          setMessage(data.error || 'Wystąpił błąd podczas weryfikacji konta.');
        }
      } catch (error) {
        ...
        <div className="max-w-md mx-auto px-4">
          <AnimatedElement variants={fadeInUp} className="text-center mb-12">
            ...
            <p className="text-lg text-[#666666]">
              Weryfikujemy Twoje konto w Programie Partnerskim ARTSCORE
            </p>
          </AnimatedElement>
              ...
              <div className="text-center space-y-6">
                <div className="flex justify-center">
                  <div className="w-20 h-20 bg-[#b38a34]/10 rounded-full flex items-center justify-center">
                    <Loader2 className="w-12 h-12 animate-spin text-[#b38a34]" />
                  </div>
                </div>
                <div className="space-y-4">
                  ...
                  <p className="text-[#666666] leading-relaxed">
                    Proszę czekać, trwa proces weryfikacji.
                  </p>
                </div>
              </div>
            )}
